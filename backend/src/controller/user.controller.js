...
export async function sendingFriendRequest(req, res) {
    try {
        ...
        res.status(201).json(createFriendRequest)
    } catch (error) {
...
export async function acceptingFriendRequest(req, res) {
    try {
...
export async function rejectFriendRequest(req, res) {
    try {
        ...
        const friendRequest = await FriendRequest.findById(requestId);
        console.log("Found friendRequest:", friendRequest);
...
export async function incomingFriendRequests(req, res) {
    try {
        const rawIncomingReq = await FriendRequest.find({
            recipient: req.user.id,
            status: "pending"
        }).populate("sender", "fullname profilePic nativeLanguage learningLanguage location");

        const rawAcceptedReq = await FriendRequest.find({
            sender: req.user.id,
            status: "accepted"
        }).populate("recipient", "fullname profilePic");

        
        const incomingReq = rawIncomingReq.filter(req => req.sender !== null);
        const acceptedReq = rawAcceptedReq.filter(req => req.recipient !== null);
